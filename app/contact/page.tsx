...
export default function ContactPage() {
  const { info, social, socialSection, map, addressFull } = contactData;
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {info.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className="group hover:border-maroon-theme flex flex-col items-center border-2 border-gray-200 bg-white p-6 text-center transition-all duration-300"
                >
                  <div className="mb-3 flex h-12 w-12 items-center justify-center">
                    <Icon className="text-maroon-theme h-6 w-6" />
                  </div>

                  <h3 className="text-black-theme mb-2 text-lg font-extrabold">
                    {item.title}
                  </h3>
                  ...
                  <div className="flex flex-col space-y-1">
                    {item.details.map((detail, idx) =>
                      'link' in detail ? (
                        ...
                        <p key={idx} className="text-base font-bold text-gray-600">
                          {detail.text}
                        </p>
                      ),
                    )}
            ...
            <div className="mb-8 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
              <div>
                <h2 className="text-black-theme mb-2 text-4xl font-extrabold">
                  แผนที่และที่อยู่ของเรา
                </h2>
                <p className="text-lg font-bold text-gray-600">{addressFull}</p>
              </div>
              <a
          ...
          <div className="flex justify-center gap-6">
            {social.map((item) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`flex h-14 w-14 items-center justify-center rounded-full text-white transition-transform hover:scale-110 ${socialBg[item.platform]}`}
